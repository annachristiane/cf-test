...
def delta_update (fieldnames, table,conn_mongo, conn):
    ...
    client = connect_to_mongo(conn_mongo)
    db = client[conn_mongo['database']]
    collection = db[conn_mongo['collection']]
    dateExec = datetime.datetime.now()
    mostRecentDate = postgres_mostRecentDate(conn,table).at[0, 'max']
    pipeline = []
    match_stage = {
    "$match": {
        "$or": [
            {"objectInfos.creation.date": {"$gte": mostRecentDate, "$lte": dateExec}},
            {"objectInfos.lastUpdate.date": {"$gte": mostRecentDate, "$lte": dateExec}}
        ]
    }
}
    pipeline.append(match_stage)
...
def insert_update_value(conn, df, table, table_schema, pk):
    ...
    for i, row in df.iterrows():
        query =  query = f"INSERT INTO {table_schema} (" + ", ".join(columns2) + ") VALUES(" + ", ".join(["%s"] * len(columns2)) + ") ON CONFLICT(" + pk + ") DO UPDATE SET " + ", ".join([f"{col} = EXCLUDED.{col}" for col in columns2[1:]])
        print(query)

...
conn = postgresql.connect_to_postgres(config.postgresql)
liste = columns_to_list(config.table_name, config.postgresql)
...
postgresql.insert_value(conn, df, config.table_schema)
...
insert_update_value(conn, df3, config.table_name,
